from utils.scrapper import get_barchart_downloads, create_bc_session
...
path = f'utils/{CONTRACT}'

# Ensure the directory exists
os.makedirs(path, exist_ok=True)

print(CONTRACT_MAP['GOLD'])
print(CONTRACT_MAP['SILVER'])
# Define contract map
CONTRACTS = {
    'GOLD': CONTRACT_MAP['GOLD'],
    'SILVER': CONTRACT_MAP['SILVER'],
}

# Create a session with the Barchart credentials
session = create_bc_session(
    barchart_username="",
    barchart_password=""
)

# Download data from Barchart
...
# Define month-to-contract code mapping
CTR_MAP = {8: 'Q', 9: 'U', 10: 'V', 11: 'X', 12: 'Z'}
...
for p in listdir(path):
    ...
    year = int(p.split('_')[2][:4])
    month = int(p.split('_')[2][4:6])

    # Read the CSV file
    df = pd.read_csv(f"{path}/{p}", index_col=0, parse_dates=True)
    #df['Contract'] = f"{CONTRACT_MAP[CONTRACT]['code']}{year}{CTR_MAP[month]}"
    data.append(df)
...
plt.savefig(f'{path}/futures_data_plot.png')
plt.close()

# Save the processed data to an Excel file
excel_path = f'{path}/Futures_Market_Data.xlsx'
